...
public class Administrator implements FileStorage{
	...
	public void editBus(Bus bus, Bus newBus){
	    ...
	    try {
	        ...
	        String sql = "UPDATE bus SET departure_time = ?,"
	        		+ "arrival_time = ?, bus_ticket_price = ?, number_of_seats = ? "
	        		+ "WHERE bus_id = ?";
			...
			BigDecimal price = BigDecimal.valueOf(newBus.getBusTicketPrice()).setScale(2, 
	...
	// remove "Driver driver" parameter
	public void addDriver(BusDriver driver){
		// refine
		this.driver = driver;
	}

	public void editDriverInfo(BusDriver driver){
		this.driver = driver;
	}
	...
	public BusDriver searchAssignedBus(int assignedBusId) {
		...
		try {
			...
			String sql = "SELECT * from bus_driver "
					+ "WHERE assigned_bus = ?";
			...
			while (resultset.next()) {
				...
				String addressSql = "SELECT * FROM address "
						+ "WHERE address_id = ? ";
	...
	public void store() {
		...
		try {
			...
			routeInsert = connection.prepareStatement("INSERT INTO route (source, destination)"
					+ " VALUES(?, ?)", Statement.RETURN_GENERATED_KEYS);
				
			busInsert = connection.prepareStatement("INSERT INTO bus (driver_id, route_id, "
					+ "departure_time, arrival_time, bus_ticket_price, number_of_seats) "
					+ "VALUES(?, ?, ?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
			...
			BigDecimal price = BigDecimal.valueOf(bus.getBusTicketPrice()).setScale(2, 
					RoundingMode.HALF_UP);
	...
	public void saveDriver(){
		...
		try {
			...
			driverInsert = connection.prepareStatement("INSERT INTO bus_driver ("
