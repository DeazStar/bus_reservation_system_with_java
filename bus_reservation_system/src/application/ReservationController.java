...
public class ReservationController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		/*
		 * Parent root; try { root = loader.load(); } catch (IOException e) {
		...
		// persists to open a new page so if the customer click the x button the program
		// will exitd==Lo
		/*
		 * if (this.customer == null) { Platform.exit(); System.exit(0); }
		 */
...

					{
						editButton.setOnAction((ActionEvent event) -> {
							...
							try {
								...
								alert.setContentText("could not reserve a seat in the bus");
								alert.showAndWait();
							}
